...
def make_ax(ax_dict: object) -> object:
    gs = ax_dict['fig'].add_gridspec(4, 4)
    hfmt = dates.DateFormatter('%m/%d')
    ax = ax_dict['fig'].add_subplot(gs[1:, :])
    pyplot.xticks(rotation='45')
    ax.xaxis.set_major_formatter(hfmt)
    line = ax.plot(ax_dict['x1'], ax_dict['y1'], marker='o', linestyle='-', color='blue', markersize=2.0,
                   label=ax_dict['label1'])
    # ax.plot(fds, heat_index, c='r')
    if ax_dict['x2'] is not None:
        ax.plot(ax_dict['x2'], ax_dict['y2'], marker='o', linestyle='', color='orange', markersize=2.0,
                label=ax_dict['label2'])
    ax.axis(ymin=10, ymax=110, xmin=math.trunc(dates.date2num(datetime.today())) - 7,
            xmax=math.trunc(dates.date2num(datetime.now())) + 1)
    ax.legend()
