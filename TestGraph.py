...
#    database_table = Settings.database_table
    ...
    query = 'SELECT Date, Temp, HI, Humid FROM OneMonth ORDER BY Date ASC'  # SELECT every 10th
    try:
...
"""
    ...
    make_fig_1(time_now)  # scope of fig is one_day()
...
def make_fig_1(time_now):
    ...
    wind_dict = get_wind(figure_1)
    temp_dict = get_temperature_data(figure_1)
    rain_dict = get_rain(figure_1)
    make_ax1(temp_dict)
    make_ax2(wind_dict)
    make_ax3(bp_dict)
    make_ax4(rain_dict)
    ...
    figure_2 = pyplot.figure(num=102, facecolor='blue')  # scope of figure is make_fig()
    pyplot.figtext(0.75, 0.95, f"{time_now}\n", fontsize=20, horizontalalignment='left', verticalalignment='top')
    temp_dict['title'] = '2'
    # change fig in dicts
    ...
    figure_3 = pyplot.figure(num=103, facecolor='red')  # scope of figure is make_fig()
    pyplot.figtext(0.75, 0.95, f"{time_now}\n", fontsize=20, horizontalalignment='left', verticalalignment='top')
    temp_dict['title'] = '3'
